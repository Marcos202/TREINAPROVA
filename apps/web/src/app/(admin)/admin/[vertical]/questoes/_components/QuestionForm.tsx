...
export function QuestionForm({ tenant, subjects, initialData, onSuccess, onCancel }: Props) {
  ...
  const [flashcardFront, setFlashcardFront] = useState(initialData?.flashcard_front ?? '');
  const [flashcardBack, setFlashcardBack] = useState(initialData?.flashcard_back ?? '');
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const payload = {
      ...
      flashcard_front: flashcardFront || null,
      flashcard_back: flashcardBack || null,
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8">
      ...
      {/* ── Section 5: Conteúdo para Flashcards ── */}
      <section className="bg-white border border-slate-200 shadow-sm p-6 rounded-2xl space-y-6">
        <div className="border-b border-slate-100 pb-3">
          <h3 className="text-sm font-semibold text-slate-900 uppercase tracking-wider">
            Conteúdo para Flashcards
          </h3>
          <p className="text-xs text-slate-400 mt-1">
            Conteúdo de memorização exibido para o aluno após errar a questão. Use HTML rico para destacar termos-chave.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
          ...
          <div className="bg-slate-50/60 border border-slate-200 rounded-xl p-4 space-y-3">
            <div className="flex items-center gap-2">
              <span className="inline-flex items-center justify-center w-6 h-6 rounded-full bg-slate-900 text-white text-[10px] font-bold shrink-0">
                F
              </span>
            ...
            <p className="text-xs text-slate-500">
              A pergunta concisa que o aluno verá primeiro. Ex: <em>"Qual a primeira conduta em choque séptico?"</em>
            </p>
            <RichTextEditor
              tenant={tenant}
              value={flashcardFront}
              onChange={setFlashcardFront}
              placeholder="Escreva a pergunta de memorização..."
              minHeight={140}
            />
          </div>
          ...
          <div className="bg-slate-50/60 border border-slate-200 rounded-xl p-4 space-y-3">
            ...
            <p className="text-xs text-slate-500">
              A resposta direta e memorável. Use <strong>negrito</strong> para destacar os termos mais importantes.
            </p>
            <RichTextEditor
              tenant={tenant}
              value={flashcardBack}
              onChange={setFlashcardBack}
              placeholder="Escreva a resposta / conceito central a memorizar..."
              minHeight={140}
            />
          </div>
        </div>
