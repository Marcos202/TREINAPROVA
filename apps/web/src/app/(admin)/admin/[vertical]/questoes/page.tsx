...
export default async function AdminQuestoesVerticalPage({ params }: Props) {
  const { vertical } = await params;

  if (!VALID_TENANTS.includes(vertical)) {
    notFound();
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <StatCard label="Vertical" value={label} suffix={vertical} isText />
      </div>
      ...
      {subjects.length === 0 && (
        <div className="bg-amber-50 border border-amber-200 rounded-lg px-4 py-3 flex items-start gap-3">
          ...
          <p className="text-sm text-amber-800">
            ...
            <code className="font-mono bg-amber-100 px-1 rounded text-xs">tenant_id = &apos;{vertical}&apos;</code>.
          </p>
        </div>
      )}
